...
test.describe('Carga de página', () => {
  ...
  test('No hay Google Fonts CDN en el HTML', async ({ page }) => {
    ...
    // Único stylesheet local: {base}/src/styles/main.css (sufijo fijo; el prefijo
    // depende del base de Vite, / o /poo-js/ en GH Pages).
    ...
    await expect(icons.nth(1)).toHaveAttribute(
      'href',
      /\/favicon\/favicon\.svg$/
    );
    await expect(
